...
export default function ChatPage() {
  ...
  // Initialize Native Browser Speech-to-Text
  useEffect(() => {
    const SpeechRecognition = window.SpeechRecognition || (window as any).webkitSpeechRecognition
    if (SpeechRecognition) {
      recognitionRef.current = new SpeechRecognition()
      
      // Stop listening automatically when the user pauses
      ...
      recognitionRef.current.onresult = (event: any) => {
        ...
        if (finalTranscript) {
          ...
          setUsedVoice(true)
          setTriggerAutoSend(true) // Trigger instant send
        }
      }
  ...
  const toggleListening = () => {
    // BROWSER UNLOCK TRICK: Play a silent sound to prove user interaction and unlock Text-to-Speech
  ...
  const sendMessage = async (overrideMessage?: string) => {
    ...
    // Flag the NEXT AI message to automatically read itself out loud if voice was used
    const isVoiceTriggered = usedVoice && !overrideMessage;
    setAutoPlayNext(isVoiceTriggered) 
  ...
  // AUTO-SUBMIT LOGIC: Instantly send when a final voice transcript is generated
  useEffect(() => {
    if (triggerAutoSend && input.trim() && !streaming) {
      setTriggerAutoSend(false);
      sendMessage();
    }
  }, [triggerAutoSend, input, streaming]);
          ...
          <>
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-6">
              {activeConv?.messages.map((msg, index) => {
                // Determine if this is the latest AI message to trigger auto-play
                ...
                <div className="flex gap-2 items-end">
                  <textarea
                    ...
                    onChange={(e) => {
                      ...
                      setUsedVoice(false); // Manual typing cancels the voice auto-play
                      setTriggerAutoSend(false);
                    }}
...
function MessageBubble({ 
  ...
  // Native Browser Text-to-Speech Function
  const playSpeech = () => {
    if (!('speechSynthesis' in window)) return
    
    window.speechSynthesis.cancel() // Stop any previous speech
    
    // Add a tiny delay to ensure the browser clears the audio queue completely
    setTimeout(() => {
